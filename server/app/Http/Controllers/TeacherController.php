...
class TeacherController extends Controller
{
    ...
    public function allStudents()
    {
        ...
        $students = studentClass::where($rules)->get();
        foreach ($students as $student) :
            $student->student;
//            $student->student->attendence;
        //        $student->section;
        //        $student->student_class;
        endforeach;
...
//dd(Auth::user()->get_user_class->get_class->year);
        $rules =[
           ['class_name','!=','*'],
           ['section_id','=',Auth::user()->get_user_class->section_id],
...
//        $classes = sectionClasses::where($rules)->get();
        $classes = sectionClasses::where($rules)->get();

    return response()->json(['status' => 200, 'classes' =>$classes,]);
}




public function getClassStudent($uid){
//dd(Auth::user()->get_user_class->get_class->year);
    if (empty($uid)):
        return response()->json(['status' => 400, 'message' => 'Bad Request'], 400);
        endif;
//    Getting class details using uuid
    $class = sectionClasses::where('uuid','=',$uid)->first('id');
...
//
    ...
    public function globalAttendance(){
